...
post* thread::get_post(int post_N){
    return Posts[post_N].get_post();
}
...
forum::forum(const string& Title){
    ...
    TheTree = new Tree[GLOBAL_F_SIZE];
...
void forum::to_Tree(thread& CurrThread,int CurrTree){
    post* Posts = CurrThread.get_posts();
    for(int j = 0; j<GLOBAL_T_SIZE ;j++){
        TheTree[CurrTree].add_to_Tree(Posts[j].get_post());
    }
}

void forum::print_sorted(){
    to_Tree(Threads[0],0);
    for(int i = 1; i<GLOBAL_F_SIZE ;i++){
        to_Tree(Threads[i],i);
        TheTree[0].enhance(&TheTree[i]);
    }
    TheTree[0].print();
}
